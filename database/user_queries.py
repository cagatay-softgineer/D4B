...
def get_user_name_by_email(email: int):
    with get_connection() as conn:
        with conn.cursor() as cur:
...
def store_user_refresh_token(id: int, refresh_token: str):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE users SET refresh_token = %s WHERE id = %s", (refresh_token, id)
            )
            conn.commit()
...
def get_one_user_by_email(email):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT email, name, avatar_url, status, role FROM users WHERE email = %s",
                (email,),
            )
            data = cur.fetchone()
            user = {
...
def get_all_users(page, page_size):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT email, name, avatar_url, status, role, id FROM users ORDER BY id LIMIT %s OFFSET %s",
                (page_size, offset),
            )
            users = [
                {
